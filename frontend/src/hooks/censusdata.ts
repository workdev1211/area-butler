...
import {ApiCoordinates, ApiGeojsonFeature, ApiGeometry} from "../../../shared/types/types";
...
const cleanProperties = (result: ApiGeojsonFeature[]) => {
    const CENSUS_PROPERTIES : {[key:string]: {label: string, unit: string}} = {
        ...
        "HHGroesse_D": {label: "Durchschnittliche Haushaltsgröße", unit: "Personen"},
        "Leerstandsquote": {label: "Anteil der leerstehenden Wohnungen", unit: "%"},
        "Wohnfl_Bew_D": {label: "Durchschnittliche Wohnfläche je Bewohner", unit: "m²"},
        "Wohnfl_Whg_D": {label: "Durchschnittliche Wohnfläche je Wohnung", unit: "m²"}
    }
            ...
            properties: Object.keys(r.properties)
                ...
                .map((propertyKey) => {
                    return {
                        ...
                        unit: (r.properties as any)[propertyKey] >= 0 ? CENSUS_PROPERTIES[propertyKey].unit : '',
                        value: (r.properties as any)[propertyKey] >= 0 ? (r.properties as any)[propertyKey] : 'unbekannt'
                    }
                })
        }
    })
...
export const useCensusData = () => {
    ...
    const fetchNearData = async (coords: ApiCoordinates) => {
        ...
        const result = (await post("/api/zensus-atlas/query", geo)).data as ApiGeojsonFeature[];
