...
const LatestUserRequestsDropDown: React.FunctionComponent =
  () => {
    ...
    const {requests} = userState.latestUserRequests;    

    const dropDownRef = useRef(null);
    const [showMenu, setShowMenu] = useState(false);
    useOnClickOutside(dropDownRef, () => showMenu && setShowMenu(false));
    const buttonStyles = 'btn btn-sm bg-white text-primary border-primary hover:bg-primary hover:text-white w-full sm:w-auto';
    ...
    const fillSearchParamsFromLatestRequest = (request: ApiSearch) => {
        ...
        const existingListing = listings.find(l => JSON.stringify(l.coordinates) === JSON.stringify(request.coordinates))
        if (!!existingListing) {
            searchContextDispatch({
        ...
        searchContextDispatch({
            ...
            payload: request.preferredAmenities.map(name => osmEntityTypes.find(entity => entity.name === name))
        })
    ...
    return requests?.length > 0 ? (
        <div className={dropdownClasses} ref={dropDownRef}>
            ...
            {showMenu && (<ul className="p-2 shadow menu menu-open dropdown-content bg-base-100 rounded-box overflow-y-scroll h-48">
                {requests.map(
                    (request: ApiSearch) => (
                        <li key={'latest-user-request-' + request.coordinates.lat + request.coordinates.lng}>
                            <button type="button"
                                key={'latest-user-request-item' + request.coordinates.lat+request.coordinates.lng}
                                onClick={(e) => {
