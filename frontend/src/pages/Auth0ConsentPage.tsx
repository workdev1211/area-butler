        ...
        <DefaultLayout title="Registrierung" withHorizontalPadding={true}>
            <div className="w-1/3 mx-auto pt-20">
                ...
                { inviteCodeNeeded && <p className="mt-10">Die Nutzung des Area Butler steht nur einem exklusiven Nutzerkreis zur
                    Verfügung.</p>}
                { inviteCodeNeeded && <p>Bitte geben Sie nachfolgend Ihren Einladungscode ein.</p>}
                <Formik
                    initialValues={{
                        inviteCode: '',
                        consentGiven: false,
                    ...
                    validationSchema={Yup.object({
                        inviteCode: inviteCodeNeeded ? Yup.string().required("Bitte geben Sie einen Einladungscode an.") : Yup.string(),
                        consentGiven: Yup.boolean().oneOf([true], "Ihre Zustimmung wird benötigt.")
                    })}
                    onSubmit={(values) => onSubmit(values)}
                >
                    <Form id={formId}>
                        {inviteCodeNeeded && <Input label="Einladungscode"
                               name="inviteCode"
                               type="text"
                               placeholder="XXXXXXXX" className="input input-bordered w-full"/>}
                        <div className="form-control mt-5">
                            <Checkbox name="consentGiven">
                                <div className="checkbox-container font-normal text-base">Hiermit stimme ich den <a
                                    target="_blank" className="link-primary" href="/terms">AGB</a> und den <a
                                    target="_blank"
                                    className="link-primary"
                                    href="/privacy">Datenschutzbestimmungen</a> zu.
                                </div>
                            </Checkbox>
                        </div>
                    </Form>
                </Formik>
                <button form={formId} key="submit" type="submit"
                        className={busy ? 'busy btn btn-primary mt-5' : 'btn btn-primary mt-5'}>Registrieren
                </button>
            </div>
        </DefaultLayout>)
}
